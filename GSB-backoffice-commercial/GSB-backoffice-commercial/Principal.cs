...
namespace GSB_backoffice_commercial
{
    public partial class Principal : Form
    {
    	...
    	//Validation commande, partie creation
        //Création des lignes depuis le DGV, puis de la commande et ajout en base
    	private void buttonValiderComCreaCom_Click(object sender, EventArgs e){
    		List<Ligne> lesLignes = new List<Ligne>();
    		int idCommande = Commande.idNouvelleCommande();
    		foreach(DataGridViewRow row in dataGridViewAfficherCommande.Rows){
                int idProduit = Convert.ToInt32(row.Cells[0].Value);
                int qteProduit = Convert.ToInt32(row.Cells[1].Value);
                double totalLigne=0;
                foreach(Produit leProduit in lesProduits){
                    if(leProduit.getNum()==idProduit){
                        totalLigne = leProduit.getPrixHT() * qteProduit;
                    }
                }
    			Ligne laLigne = new Ligne(
                    idProduit, 
            ...
            foreach(Ligne laLigne in lesLignes){
    			montantTotal += laLigne.getTotal();
    		}
    		
    		Commande laCommande = new Commande(idCommande,
    				lesPros.ElementAt(comboBoxProfessionnelsCreaCom.SelectedIndex).getCode(),
                    DateTime.Today,
                    checkBoxEtatCreaCom.Text,
    				montantTotal);
    
    		foreach(Ligne laLigne in lesLignes){
    			laCommande.ajouterLigne(laLigne);
    		}
    	}
